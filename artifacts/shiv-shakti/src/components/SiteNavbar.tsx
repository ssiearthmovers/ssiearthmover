...
import { Menu, X, Download, ChevronDown, Search } from "lucide-react";
  ...
  return (
    <header
      className={`fixed top-9 left-0 w-full z-50 transition-all duration-300 ${
        isScrolled
          ? "bg-[#0F1014]/97 backdrop-blur-md border-b border-[#2A2E37] py-2.5"
          : "bg-[#0F1014]/90 backdrop-blur-sm py-3"
      }`}
    >
      <div className="max-w-7xl mx-auto px-4 md:px-8 flex justify-between items-center gap-4">
        ...
        {/* Desktop nav */}
        <nav className="hidden lg:flex items-center text-xs font-bold text-gray-300 shrink-0">

          {/* Products dropdown */}
          <div className="relative">
            <button
              className="flex items-center gap-1 px-2.5 py-2 rounded hover:text-[#F5A623] transition-colors whitespace-nowrap"
          ...
          <Link href="/find-my-part" className="px-2.5 py-2 rounded hover:text-[#F5A623] transition-colors whitespace-nowrap">
            Find My Part
          </Link>

          <Link href="/search" className="flex items-center gap-1.5 px-2.5 py-2 rounded hover:text-[#F5A623] transition-colors whitespace-nowrap">
            <Search className="w-3.5 h-3.5" /> Search Parts
          </Link>

          <button onClick={() => goHome("about")} className="px-2.5 py-2 rounded hover:text-[#F5A623] transition-colors whitespace-nowrap">
            About
          </button>
          <button onClick={() => goHome("contact")} className="px-2.5 py-2 rounded hover:text-[#F5A623] transition-colors whitespace-nowrap">
            Contact
          </button>
        </nav>

        {/* Right side — CTA buttons only (phone is in ticker) */}
        <div className="hidden lg:flex items-center gap-2 shrink-0">
          <a
            href="/ssi-catalogue.pdf"
            download="SSI-Earthmovers-Catalogue.pdf"
            className="flex items-center gap-1.5 border border-[#F5A623]/50 text-[#F5A623] px-3 py-2 rounded text-xs font-bold hover:bg-[#F5A623]/10 transition-colors whitespace-nowrap"
          >
            <Download className="w-3.5 h-3.5" /> Catalogue
          </a>
          <button
            onClick={() => goHome("contact")}
            className="bg-[#F5A623] text-black px-4 py-2 rounded font-black text-xs hover:brightness-110 transition-all whitespace-nowrap"
          >
            Get a Quote
          </button>
        </div>
      ...
      {/* Mobile menu */}
      {mobileMenuOpen && (
        <div className="lg:hidden bg-[#0F1014] border-t border-[#2A2E37] px-6 py-6 flex flex-col gap-1 max-h-[80vh] overflow-y-auto">
          ...
          <div className="mt-4 flex flex-col gap-3">
            ...
            <Link href="/find-my-part" onClick={() => setMobileMenuOpen(false)} className="block py-2.5 text-gray-300 font-semibold hover:text-[#F5A623] transition-colors">
              Find My Part
            </Link>
